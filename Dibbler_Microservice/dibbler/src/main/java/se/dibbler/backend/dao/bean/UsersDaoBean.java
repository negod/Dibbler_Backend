 ...
 */
@Stateless
public class UsersDaoBean extends BaseDaoBean<User, UsersDto> implements UsersDao<User, UsersDto> {

    public UsersDaoBean() {
        super(User.class, UsersDto.class);
    }

    @Override
    public Response create(UsersDto dto) {
        Response<User> entity = super.mapFromDtoToEntity(dto);
        if (entity.hasErrors) {
    ...
    public Response<UserSummaryDto> getUserSummaryById(String id) {
        Response<User> entity = super.getByExtId(id);
        if (entity.hasErrors) {
            return Response.error(entity.getError());
        }
    ...
    public Response<List<UserSummaryDto>> getAllUserSummary() {
        Response<List<User>> entityList = super.getAll();
    ...
    public Response<UserSummaryDto> authenticate(String username, String password) {
        ...
        Response<User> user = super.getSingleByNamedQuery(DibblerNamedQueries.USERS_AUTHENTICATE, params);
        if (user.hasErrors) {
    ...
    public Response<UserSummaryDto> getUserByFacebookId(String facebookId) {
        ...
        Response<User> user = super.getSingleByNamedQuery(DibblerNamedQueries.USERS_FINDBY_FACEBOOK_ID, params);
        if (user.hasErrors) {
            return Response.error(user.getError());
        }
        return UserSummaryMapper.getInstance().mapFromEntityToDto(user.getData());
    ...
    public Response<UserSummaryDto> getUserByGoogleId(String googleId) {
        ...
        Response<User> user = super.getSingleByNamedQuery(DibblerNamedQueries.USERS_FINDBY_GOOGLE_ID, params);
        if (user.hasErrors) {
            return Response.error(user.getError());
        }
        return UserSummaryMapper.getInstance().mapFromEntityToDto(user.getData());
